<?php if (!isset($velocityData)): ?>
    <form action="<?= $request->getBaseUrl() ?>" method="post" enctype="multipart/form-data">
        <div class="row">
            <div class="col-xl" style="height: 90px;">
                <label for="file">1. CSV file exported from Jira</label>
                <div class="custom-file">
                    <input type="file" class="custom-file-input" id="file" name="<?= \Model\Request::PARAM_FILE ?>">
                    <label class="custom-file-label" for="file">Choose file</label>
                </div>
            </div>
            <div class="col-xl" style="height: 90px;">
                <label for="days">2. Number of working days to forecast for:</label>
                <input type="text" class="form-control" id="days" name="<?= \Model\Request::PARAM_DAYS ?>" placeholder="10 equals two week sprint" value="10" required>
            </div>
            <div class="col-xl" style="height: 90px;">
                <label for="metric">3. Forecast metric</label>
                <select class="custom-select d-block w-100" id="metric" name="<?= \Model\Request::PARAM_METRIC ?>">
